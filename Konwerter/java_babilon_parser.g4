...
variableDefinition : (INTEGER_TOKEN | FLOAT_TOKEN | STRING_TOKEN | BOOLEAN) IDENTIFIER (ASSIGN expression)? SEMICOLON;
...
primaryExpression : ((MINUS)? INTEGER | (MINUS)? FLOAT | STRING | methodCalling | fieldAccessing | arrayGet | IDENTIFIER | NULL | TRUE | FALSE | LPAREN expression RPAREN);
...
listDefinition : ARRAY_LIST LESS (INTEGERB | DOUBLEB | FLOATB | LONGB | SHORTB | BYTEB | CHARACTERB| STRING_TOKEN | BOOLEANB) GREATER IDENTIFIER (ASSIGN NEW ARRAY_LIST LESS GREATER LPAREN RPAREN)? SEMICOLON;
